const Pages = {
  ...
  /* ================================================================
     PROMO POPUP
     ================================================================ */
  renderPromoPopup: function(container) {
    var db = ArcanoDB.getDB();
    var promo = (db && db.tiendaConfig && db.tiendaConfig.promoPopup) || {};
    var isActive = promo.active === true;
    var imgDesk = promo.imagenDesktop || '';
    var imgMob = promo.imagenMobile || '';
    var link = promo.link || '';

    var h = '<div class="card"><div class="card-header"><h3>Promo Popup</h3>' +
      '<p class="text-xs text-muted">Configura un popup promocional que se muestra al ingresar a la tienda. Puedes subir una imagen para escritorio y otra para mobile/tablet.</p></div>' +
      '<div class="card-body">';

    // Toggle activo
    h += '<div class="form-group"><label>Estado</label>' +
      '<button class="btn ' + (isActive ? 'btn-green' : 'btn-outline') + '" id="promo-toggle" onclick="Pages._togglePromo()">' + (isActive ? 'ACTIVADO' : 'DESACTIVADO') + '</button>' +
      '</div>';

    // Link opcional
    h += '<div class="form-group"><label>Link (opcional)</label>' +
      '<input class="input" id="promo-link" value="' + link.replace(/"/g, '&quot;') + '" placeholder="https://...">' +
      '<p class="text-xs text-muted mt-4">Si el usuario hace clic en la imagen, ira a este enlace. Deja vacio para solo cerrar.</p></div>';

    // Imagen Desktop
    h += '<div class="form-group"><label>Imagen Escritorio</label>' +
      '<p class="text-xs text-muted">Se muestra en pantallas mayores a 768px. Recomendado: 1200x800 px.</p>' +
      '<div class="img-upload-area" id="img-area-promo-desk">' +
      '<input type="file" accept="image/*" id="f-promo-desk" style="display:none" onchange="Pages._handlePromoImg(this,\'desktop\')">';
    if (imgDesk) {
      h += '<img src="' + imgDesk + '" class="img-preview" id="img-preview-promo-desk">' +
        '<button class="btn btn-sm btn-red" style="margin-top:6px" onclick="Pages._removePromoImg(\'desktop\')">Quitar</button>';
    }
    h += '<div class="img-upload-placeholder" onclick="document.getElementById(\'f-promo-desk\').click()"><span>+ Imagen Escritorio</span></div>' +
      '</div></div>';

    // Imagen Mobile
    h += '<div class="form-group"><label>Imagen Mobile / Tablet</label>' +
      '<p class="text-xs text-muted">Se muestra en pantallas de 768px o menos. Recomendado: 800x1200 px (vertical).</p>' +
      '<div class="img-upload-area" id="img-area-promo-mob">' +
      '<input type="file" accept="image/*" id="f-promo-mob" style="display:none" onchange="Pages._handlePromoImg(this,\'mobile\')">';
    if (imgMob) {
      h += '<img src="' + imgMob + '" class="img-preview" id="img-preview-promo-mob">' +
        '<button class="btn btn-sm btn-red" style="margin-top:6px" onclick="Pages._removePromoImg(\'mobile\')">Quitar</button>';
    }
    h += '<div class="img-upload-placeholder" onclick="document.getElementById(\'f-promo-mob\').click()"><span>+ Imagen Mobile</span></div>' +
      '</div></div>';

    // Boton guardar link
    h += '<div class="form-group mt-16"><button class="btn btn-gold" onclick="Pages._savePromoLink()">Guardar Link</button></div>';

    // Preview
    h += '<div class="form-group mt-16"><label>Vista previa</label>' +
      '<p class="text-xs text-muted">Previsualiza como se vera el popup en la tienda.</p>' +
      '<button class="btn btn-outline" onclick="Pages._previewPromo()">Abrir Preview</button></div>';

    h += '</div></div>';
    container.innerHTML = h;
  },

  _togglePromo: function() {
    var db = ArcanoDB.getDB();
    var promo = (db && db.tiendaConfig && db.tiendaConfig.promoPopup) || {};
    var isActive = promo.active === true;
    writeField('tiendaConfig/promoPopup/active', !isActive);
    App.renderPage('promopopup');
  },

  _savePromoLink: function() {
    var input = document.getElementById('promo-link');
    if (!input) return;
    writeField('tiendaConfig/promoPopup/link', input.value.trim());
    toast('Link guardado');
  },

  _handlePromoImg: function(input, tipo) {
    if (!input.files || !input.files[0]) return;
    var file = input.files[0];
    var maxW = tipo === 'desktop' ? 1200 : 800;
    ArcanoDB.compressImage(file, maxW, 0.8, function(err, dataUrl) {
      if (err) { alert('Error: ' + err); return; }
      writeField('tiendaConfig/promoPopup/imagen' + (tipo === 'desktop' ? 'Desktop' : 'Mobile'), dataUrl);
      App.renderPage('promopopup');
    });
  },

  _removePromoImg: function(tipo) {
    writeField('tiendaConfig/promoPopup/imagen' + (tipo === 'desktop' ? 'Desktop' : 'Mobile'), '');
    App.renderPage('promopopup');
  },

  _previewPromo: function() {
    var db = ArcanoDB.getDB();
    var promo = (db && db.tiendaConfig && db.tiendaConfig.promoPopup) || {};
    if (!promo.imagenDesktop && !promo.imagenMobile) { alert('No hay imagenes cargadas'); return; }
    var isMobile = window.innerWidth <= 768;
    var imgSrc = isMobile ? promo.imagenMobile : promo.imagenDesktop;
    if (!imgSrc) imgSrc = promo.imagenDesktop || promo.imagenMobile;
    if (!imgSrc) { alert('No hay imagen para este tamano'); return; }
    ...
    closeBtn.style.cssText = 'position:absolute;top:12px;right:16px;color:#fff;font-size:24px;background:none;border:none;cursor:pointer';
